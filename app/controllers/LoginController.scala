...
class LoginController @Inject() (cc: ControllerComponents, val dbConfigProvider: DatabaseConfigProvider, val tableDefs: TableDefs)(
  ...
  def registerForm: Action[AnyContent] = Action { implicit request =>
    Ok(views.html.forms.registerForm(FormMappings.registerValuesForm))
  }
  ...
  def register: Action[AnyContent] = Action.async { implicit request =>
    ...
    val onRead: RegisterFormValues => Future[Result] = { credentials =>
      ...
      val pwHash  = UserPassword(credentials.username, credentials.pw.boundedBcrypt)

      tableDefs.futureInsertUser(newUser) flatMap {
        case false => Future(BadRequest("Could not save user!"))
        case true =>
          tableDefs.futureSavePwHash(pwHash) map { _ =>
  ...
  def loginForm: Action[AnyContent] = Action { implicit request =>
    Ok(views.html.forms.loginForm(FormMappings.loginValuesForm))
  }
  ...
  def changePwForm: EssentialAction = futureWithUser { user => implicit request =>
    tableDefs.futurePwHashForUser(user).map(_.isDefined) map { hasPw =>
      Ok(views.html.forms.changePwForm(user, FormMappings.changePwForm, hasPw))
    }
  }

  def changePw: EssentialAction = futureWithUser { user => implicit request =>
    tableDefs.futurePwHashForUser(user) flatMap { maybePwHashForUser: Option[UserPassword] =>
      ...
      def onRead: ChangePwFormValues => Future[Result] = { changePwFormValues: ChangePwFormValues =>
        ...
        if (oldPwCorrect && newPwsEqual) {
          tableDefs.futureUpdatePwHashForUser(user, changePwFormValues.firstNewPw.boundedBcrypt) map {
            case false => ???
